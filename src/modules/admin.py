...
def ignore(prefix, chan, params):
    bot.config.append('ignore', params[0])

@hook.command('unignore', perm='staff')
def unignore(prefix, chan, params):
    bot.config.remove('ignore', params[0])
...
@hook.command('help')
def helpmsg(prefix, chan, params):
    commands = []
    prefix_j = '%s!%s@%s' % (prefix[0], prefix[1], prefix[2])

    for command in bot.commands:
        for func in bot.commands[command]:
            if hasattr(func, '_perm'):
                perm = getattr(func, '_perm')

                if perm not in bot.config.get('perms', {}):
                    continue

                if not bot.match(bot.config.get('perms', {})[perm], prefix_j):
                    continue

            if command not in commands:
                commands.append(bot.config.get('prefix') + command)

    if commands:
        commands.sort()
        bot.say(prefix[0], ', '.join(commands))
